  ...
  try {
    ...
    if (containerRunId != null) {
      completeContainerRun(containerRunId, {
        ...
        exit_code: output.status === 'success' ? 0 : 1,
        status: output.timedOut && output.status === 'error' ? 'timeout' : output.status === 'success' ? 'success' : 'error',
        result: output.result?.slice(0, 200) ?? null,
...
async function main(): Promise<void> {
  ...
  // Shared callback: refresh tasks snapshot for all groups
  ...
  if (DASHBOARD_ENABLED) {
    startDashboard({
      ...
      computeNextRun,
      onTasksChanged,
      runTaskNow: (task) => enqueueManualRun(task, schedulerDeps),
      getActiveContainers: () => queue.getActiveContainers(),
      getQueueStatus: () => queue.getQueueStatus(),
      stopActiveContainer: (jid) => queue.stopContainer(jid),
      getContainerRuns: (limit, groupFolder) =>
        getContainerRuns(limit, groupFolder),
